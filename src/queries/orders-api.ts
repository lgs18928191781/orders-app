...
export const getRawTx = async (txid: string, net: string) => {
  const res = await ordersCommonApiFetch(`tx/raw?net${net}&txId=${txid}`)
  if (!res) return ''
  return res
}

export const getBrc20Faucet = async (params: {
  addressType: string
  publicKey: string
...
export const getListingUtxos: (address?: string) => Promise<
  {
    order: string
    dummyId: string
    timestamp: number
