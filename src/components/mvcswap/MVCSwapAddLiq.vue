    ...
    <div v-if="curPair">
        <div class="swap-sub-control-panel">
            ...
            <div class="flex h-16 items-center justify-between space-x-2">
                <input type="number" v-model="token1Amount" @input="handelToken1Change"
                    class="quiet-input w-12 flex-1 bg-transparent p-0 leading-loose" placeholder="Enter an amount">
        ...
        <div class="swap-sub-control-panel">
            ...
            <div class="flex h-16 items-center justify-between space-x-2">
                <input type="number" v-model="token2Amount" @input="handelToken2Change"
                    class="quiet-input w-12 flex-1 bg-transparent p-0 leading-loose" placeholder="Enter an amount" />
                <div class="flex items-center gap-1 rounded-full bg-zinc-900 p-1 px-4 text-xl">
        ...
        <div class="grid grid-rows-2 gap-y-1 mt-5 text-sm">
...
import { formatAmount, formatSat } from '@/utils';
import { countLpAddAmount, countLpAddAmountWithToken2, formatTok } from '@/utils/mvcswap';
...
const priceLabel = computed(() => {
    ...
    const _swapToken1Amount = formatSat(curPair.value.token1Amount, curPair.value.token1.decimal);
    const _swapToken2Amount = formatSat(curPair.value.token2Amount, curPair.value.token2.decimal);
...
const handelToken1Change = (e:any) => {
    ...
    if (!curPair.value) return
    const { token1, token2, swapToken1Amount = '', swapToken2Amount = '', swapLpAmount = '' } = curPair.value
...
const handelToken2Change = (e:any) => {
    ...
    const user_origin_amount = formatSat(token1AddAmount.toString(), token1.decimal);
...
const handleSubmit = async () => {
    ...
    try {
        ...
        const { swapToken1Amount, swapToken2Amount, swapLpAmount, mvcToAddress, tokenToAddress, requestIndex, txFee } = data;
        ...
        if (isMvc && BigNumber(token1Amount.value)
        ...
        if (isMvc) {
            const tx_res:any = await window.metaidwallet.transfer({
                ...
                tasks: [
                    {
                        type: 'space',
                        receivers: [{ address: mvcToAddress, amount: (BigInt(_origin_amount.toString()) + BigInt(txFee)).toString() }],
                    },
                    {
                        type: 'token',
                        codehash: token2.codeHash,
                        genesis: token2.tokenID,
                        receivers: [{ address: tokenToAddress, amount: _aim_amount.toString() }],
                    }
                ],
            });
            ...
            const tx_res:any = await window.metaidwallet.transfer({
                ...
                tasks: [
                    ...
                    },
                    {
                        ...
                        receivers: [{ address: tokenToAddress, amount: _origin_amount.toString() }],
                    },
                    {
                        type: 'token',
                        codehash: token2.codeHash,
