...
<template>
  ...
  >
    <span
      class="group-disabled:bg-zinc-800! absolute inset-[-1000%] animate-[spin_2s_linear_infinite] group-enabled:bg-[conic-gradient(from_90deg_at_50%_50%,#38bdf8_0%,#4f46e5_30%,#38bdf8_100%)]"
    ></span>
    <span
      ...
      :class="[
        danger ? 'text-red-500' : 'text-mvc group-hover:text-orange-100',
      ]"
    >
      <Loader2Icon
        class="ml-2 h-6 w-6 animate-spin text-zinc-500"
        v-if="props.submiting"
      />
      <span v-if="text">
        {{ text }}
      </span>
