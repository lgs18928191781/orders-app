...
import { type SwapYieldBrc20History } from '@/queries/swap-yield-reward-history.query'

const props = defineProps<{
  record: SwapYieldBrc20History
}>()
  ...
  <div class="rounded-lg border border-primary/15 bg-zinc-950 px-4 py-4">
    <h3 class="flex items-center justify-between">
      <div class="flex items-center text-lg">
        <span class="font-bold text-primary" v-if="record.realReward">
          {{ prettyCoinDisplay(record.realReward, record.tick) }}
        </span>
    ...
    <div class="mt-4 flex items-center justify-between">
      <div class="space-y-2 text-sm">
        ...
        <div class="flex items-center">
          <span class="inline-block w-40 shrink-0 text-zinc-500"> # Day </span>
          <span>
            {{ record.dailyCount || '-' }}
          </span>
        </div>
